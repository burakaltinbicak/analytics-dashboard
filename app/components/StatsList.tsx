            ...
            <section>
                ...
                <div className="flex flex-col gap-3">
                    {sortByDate(stats.sessions).map((session: Session) => (
                        <div key={session.id} className="bg-white border border-[#4a7c59]/5 rounded-2xl px-6 py-4 flex items-center justify-between shadow-[0_4px_20px_rgba(74,124,89,0.03)] hover:shadow-[0_4px_25px_rgba(74,124,89,0.06)] transition-all group">
                            <div className="flex items-center gap-6">
                                {/* Browser/OS Icon Placeholder */}
                                <div className="w-10 h-10 bg-[#fdfcf8] border border-[#e7e5e0] rounded-xl flex items-center justify-center text-[#5c5c56] group-hover:bg-[#4a7c59] group-hover:text-white group-hover:border-transparent transition-all">
            ...
            <section>
                ...
                <div className="flex flex-col gap-3">
                    {sortByDate(stats.events).map((event: SiteEvent) => (
                        <div key={event.id} className="bg-white border border-[#4a7c59]/5 rounded-2xl px-6 py-5 shadow-sm hover:border-[#4a7c59]/20 transition-all">
                            ...
                            {event.event_data && (
                                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-[#f4f4f2]">
                                    {Object.entries(event.event_data)
                                        .filter(([_, v]) => v !== null)
                                        .map(([key, value]) => (
                                            <div key={key} className="flex items-center gap-1.5 bg-[#fdfcf8] border border-[#e7e5e0] px-2.5 py-1 rounded-lg">
